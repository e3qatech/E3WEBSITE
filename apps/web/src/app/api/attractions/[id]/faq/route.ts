...
export async function GET(
  ...
  try {
    ...
    const items = await db.attractionFaq.findMany({
      where: { attractionId },
    });
    return NextResponse.json(items);
  } catch (error: any) {
    console.error('[ATTRACTIONS_FAQ_GET]', error);
...
export async function POST(
  ...
  try {
    ...
    const missing = ["questionEn","questionAr","answerEn","answerAr"].find(f => body[f] === undefined);
    if (missing) return NextResponse.json({ error: 'Missing required field: ' + missing }, { status: 400 });

    const newItem = await db.attractionFaq.create({
      data: {
        attractionId,
        ...body
      },
    ...
    console.error('[ATTRACTIONS_FAQ_POST]', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error('[ATTRACTIONS_FAQ_DELETE]', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
