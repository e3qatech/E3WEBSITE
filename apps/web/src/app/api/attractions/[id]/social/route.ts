...
export async function GET(
  ...
  try {
    ...
    const items = await db.attractionSocialLink.findMany({
      where: { attractionId },
    });
    return NextResponse.json(items);
  } catch (error: any) {
    console.error('[ATTRACTIONS_SOCIAL_GET]', error);
...
export async function POST(
  ...
  try {
    ...
    const missing = ["platform","url"].find(f => body[f] === undefined);
    if (missing) return NextResponse.json({ error: 'Missing required field: ' + missing }, { status: 400 });

    const newItem = await db.attractionSocialLink.create({
      data: {
        attractionId,
        ...body
      },
    ...
    console.error('[ATTRACTIONS_SOCIAL_POST]', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await db.attractionSocialLink.delete({
    ...
    console.error('[ATTRACTIONS_SOCIAL_DELETE]', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
