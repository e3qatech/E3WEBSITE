...
export async function GET(
  ...
  try {
    ...
    const items = await db.attractionPricing.findMany({
      where: { attractionId },
    });
    return NextResponse.json(items);
  } catch (error: any) {
    console.error('[ATTRACTIONS_PRICING_GET]', error);
...
export async function POST(
  ...
  try {
    ...
    const missing = ["titleEn","titleAr","price"].find(f => body[f] === undefined);
    if (missing) return NextResponse.json({ error: 'Missing required field: ' + missing }, { status: 400 });

    const newItem = await db.attractionPricing.create({
      data: {
        attractionId,
        ...body
      },
    ...
    console.error('[ATTRACTIONS_PRICING_POST]', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await db.attractionPricing.delete({
    ...
    console.error('[ATTRACTIONS_PRICING_DELETE]', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
