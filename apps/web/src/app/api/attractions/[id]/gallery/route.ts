...
export async function GET(
  ...
  try {
    ...
    const items = await db.attractionGalleryItem.findMany({
      where: { attractionId },
    });
    return NextResponse.json(items);
  } catch (error: any) {
    console.error('[ATTRACTIONS_GALLERY_GET]', error);
...
export async function POST(
  ...
  try {
    ...
    const missing = ["url"].find(f => body[f] === undefined);
    if (missing) return NextResponse.json({ error: 'Missing required field: ' + missing }, { status: 400 });

    const newItem = await db.attractionGalleryItem.create({
      data: {
        attractionId,
        ...body
      },
...
export async function DELETE(
  ...
  try {
    ...
    await db.attractionGalleryItem.delete({
