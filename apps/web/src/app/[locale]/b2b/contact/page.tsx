          ...
          <div className="grid md:grid-cols-12 gap-16">
            ...
            <div className="md:col-span-5 space-y-12">
              {(businessEmail || careersEmail || phone) && (
                <div>
                  <h3 className="text-2xl font-bold text-[var(--text-primary)] mb-6 tracking-tight">{isAr ? 'استفسارات مباشرة' : 'Direct Inquiries'}</h3>
              ...
              {submitted ? (
                <div className="p-12 rounded-2xl bg-[var(--surface-default)] border border-emerald-500/50 text-center shadow-lg">
                  ...
                  <button 
                    ...
                    className="px-6 py-3 rounded-full border border-[var(--border-level-2)] text-[var(--text-primary)] font-bold hover:bg-[var(--surface-hover)] transition-colors cursor-pointer"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-8 p-10 rounded-2xl bg-[var(--surface-default)] border border-[var(--border-level-2)] shadow-sm">
                  ...
                  <div className="space-y-4">
                    <label className="text-sm font-bold text-[var(--text-tertiary)] uppercase tracking-widest">{isAr ? 'نوع الاستفسار' : 'Inquiry Type'}</label>
                    <div className="flex flex-wrap gap-4">
                      {(isAr ? ['تقديم طلب عروض', 'أعمال عامة', 'شراكة', 'أخرى'] : ['RFP Submission', 'General Business', 'Partnership', 'Other']).map(type => (
                        <label 
                          key={type} 
                          className={`px-5 py-3 rounded-xl border cursor-pointer font-bold text-sm transition-colors ${
                            inquiryType === type 
                              ? 'border-emerald-500 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400'
                              : 'border-[var(--border-level-2)] bg-[var(--bg-level-2)] text-[var(--text-secondary)] hover:border-emerald-500/50'
                          }`}
                        >
                          <input 
                            type="radio" 
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-[var(--text-secondary)]">{isAr ? 'الاسم الكامل' : 'Full Name'}</label>
                      <input 
                        required
                        name="name"
                        type="text" 
                        className="w-full bg-[var(--bg-level-1)] border border-[var(--border-level-2)] rounded-xl px-4 py-3 text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-emerald-500 transition-colors"
                        placeholder={isAr ? 'فلان الفلاني' : 'Jane Doe'}
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-[var(--text-secondary)]">{isAr ? 'الشركة' : 'Company'}</label>
                      <input 
                        required
                        name="company"
                        type="text" 
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-[var(--text-secondary)]">{isAr ? 'البريد الإلكتروني' : 'Email Address'}</label>
                      <input 
                        required
                        name="email"
                        type="email" 
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-[var(--text-secondary)]">{isAr ? 'رقم الهاتف' : 'Phone Number'}</label>
                      <input 
                        type="tel" 
                        name="phone" 
                        className="w-full bg-[var(--bg-level-1)] border border-[var(--border-level-2)] rounded-xl px-4 py-3 text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-emerald-500 transition-colors"
                  ...
                  {/* Message */}
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-[var(--text-secondary)]">{isAr ? 'تفاصيل المشروع أو الرسالة' : 'Project Details or Message'}</label>
                    <textarea 
                      required
                      name="notes"
                      rows={5}
                      className="w-full bg-[var(--bg-level-1)] border border-[var(--border-level-2)] rounded-xl px-4 py-3 text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-emerald-500 transition-colors resize-none"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-[var(--text-secondary)]">{isAr ? 'وثيقة طلب العروض / المرفقات (اختياري)' : 'RFP Document / Attachments (Optional)'}</label>
                    <label className="block w-full border-2 border-dashed border-[var(--border-level-2)] hover:border-emerald-500/50 rounded-xl p-6 text-center transition-colors cursor-pointer bg-[var(--bg-level-1)]">
                      <input
                        type="file"
                        className="hidden"
                        accept=".pdf,.docx,.doc"
                      ...
                      {rfpFile ? (
                        <div className="flex items-center justify-center gap-3">
                          <span className="text-sm font-bold text-emerald-500">📄 {rfpFile.name} ({(rfpFile.size / 1024 / 1024).toFixed(2)} MB)</span>
                          <button
                            type="button"
                        ...
                        <div>
                          ...
                          <p className="text-xs text-[var(--text-tertiary)] mt-1">PDF, DOCX up to 25MB (Encrypted & Qatar PDPL Compliant)</p>
                        </div>
                      )}
                    </label>
                  </div>
                  ...
                  {errorMessage && (
                    <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-500 text-sm font-medium">
                      {errorMessage}
                    </div>
                  )}
                  ...
                  <button 
                    ...
                    className="w-full py-4 bg-emerald-500 text-slate-950 font-bold text-lg rounded-xl hover:bg-emerald-400 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer shadow-md"
                  >
                    {isSubmitting
