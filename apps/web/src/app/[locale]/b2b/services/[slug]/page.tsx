    ...
    <div className="flex flex-col w-full bg-zinc-950 min-h-screen" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <section className="relative min-h-[60vh] flex items-center pt-24 pb-12 overflow-hidden">
        <div className="absolute inset-0 z-0">
          {service.heroMediaUrl ? (
            <UniversalMediaRenderer 
              ...
              alt={isAr ? (service.titleAr || service.titleEn) : service.titleEn}
            />
          ) : (
        ...
        <div className="container relative z-10 mx-auto px-4 md:px-8">
          <Link href={`/${locale}/b2b/services`} className="inline-flex items-center gap-2 text-sm font-bold text-zinc-400 hover:text-emerald-400 uppercase tracking-wider mb-8">
            <ArrowRight className="w-4 h-4 rotate-180 rtl:rotate-0" /> {isAr ? 'العودة للخدمات' : 'Back to Services'}
          </Link>
          
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-zinc-100 tracking-tight mb-6 max-w-4xl">
            {isAr ? (service.titleAr || service.titleEn) : service.titleEn}
          </h1>
          {(service.taglineEn || service.taglineAr) && (
            <p className="text-xl md:text-2xl text-zinc-300 max-w-2xl font-medium">
              {isAr ? (service.taglineAr || service.taglineEn) : service.taglineEn}
            </p>
          )}
        </div>
      </section>
      ...
      <section className="py-24">
        <div className="container mx-auto px-4 md:px-8">
          <div className="grid md:grid-cols-2 gap-16 lg:gap-24">
            <div>
              <h2 className="text-3xl font-black text-zinc-100 mb-6 tracking-tight">{isAr ? 'التخصص والمجال' : 'The Discipline'}</h2>
              <div className="text-lg text-zinc-400 leading-relaxed whitespace-pre-wrap">
                {isAr ? (service.contentAr || service.contentEn) : service.contentEn}
              </div>
            </div>
            
            {processList.length > 0 && (
              <div className="p-8 rounded-xl bg-zinc-900 border border-zinc-800">
                <h3 className="text-xl font-bold text-zinc-100 mb-6">{isAr ? 'المخرجات والعملية الأساسية' : 'Core Deliverables & Process'}</h3>
                <ul className="space-y-6">
                  {processList.map((step: any, i) => (
                    <li key={i} className="flex items-start gap-4">
                      <CheckCircle2 className="w-6 h-6 text-emerald-500 shrink-0 mt-1" />
                      <div>
                        <div className="text-zinc-100 font-bold mb-1">{isAr ? (step.titleAr || step.titleEn) : step.titleEn}</div>
                        <div className="text-zinc-400 text-sm">{isAr ? (step.descAr || step.descEn) : step.descEn}</div>
                      </div>
                    </li>
                  ))}
                </ul>
