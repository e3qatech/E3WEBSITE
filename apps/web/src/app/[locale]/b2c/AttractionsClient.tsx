...
export function AttractionsClient({
  ...
  const dropdownResults = useMemo(() => {
    ...
    return attractions
      .filter((a) => {
        return (
          (a.nameEn?.toLowerCase() || "").includes(lowerSearch) ||
  ...
  const filteredAttractions = useMemo(() => {
    const result = attractions.filter((a) => {
      ...
      if (statusFilter === "Active Now")
        matchStatus = a.computedStatus === "ACTIVE";
      if (statusFilter === "Coming Soon")
        matchStatus = a.computedStatus === "COMING SOON";
        ...
        <div className="max-w-md w-full bg-slate-900/80 border border-slate-800 rounded-2xl p-8 shadow-2xl backdrop-blur-xl space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-2xl font-bold tracking-tight">
              {isAr ? "الخدمة غير متوفرة حالياً" : "System Maintenance"}
            </h1>
            <p className="text-sm text-slate-400 leading-relaxed">
              {isAr
                ? "نقوم بإجراء صيانة دورية للنظام لتحديث الخدمات. يرجى إعادة المحاولة بعد قليل."
                : "We are currently undergoing system maintenance. Please refresh or try again shortly."}
            </p>
          </div>
          <div className="pt-2 flex flex-col items-center space-y-3">
            <button
              onClick={() => window.location.reload()}
              className="w-full py-3 px-6 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-semibold transition-colors duration-200 text-sm shadow-lg shadow-emerald-500/20 cursor-pointer"
            >
            ...
            {(hero.showSearch ?? true) && (
              ...
              >
                <div className="relative flex items-center w-full">
                  <Search
                    className={`absolute ${isAr ? "end-4" : "start-4"} w-5 h-5 text-[var(--text-tertiary)] z-20`}
                  />
                  <input
                    type="search"
                    value={localSearch}
                    ...
                    placeholder={
                      isAr ? "ابحث عن التجارب..." : "Search attractions..."
                    }
                                ...
                                >
                                  ...
                                  <div className="flex flex-col">
                                    ...
                                    {attr.computedStatus === "ACTIVE" && (
                                      <span className="text-emerald-500 text-[10px] uppercase font-black tracking-wider mt-0.5">
                                        Live Now
                                      </span>
                                    )}
                                  </div>
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 mt-6">
                  ...
                  <div className="relative flex items-center">
                    ...
                    <ChevronDown
                      className={`absolute ${isAr ? "left-3" : "right-3"} w-4 h-4 text-[var(--text-tertiary)] pointer-events-none`}
                    />
                  </div>
                </div>
              </motion.div>
          ...
          {isLoading && (
            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div
                  key={i}
                  className="w-full h-80 bg-[var(--surface-default)] rounded-3xl animate-pulse border border-[var(--border-level-1)]"
                />
              ))}
            </div>
          )}
          ...
          {!isLoading && (
            <>
                ...
                <section className="w-full">
                  <div className="flex items-center gap-3 mb-8">
                    ...
                    <h2 className="text-3xl md:text-4xl font-black font-righteous uppercase tracking-wide">
                      {isAr
                        ? cmsData?.featuredTitleAr || ""
                        : cmsData?.featuredTitleEn || ""}
                    </h2>
                  </div>
                    ...
                    >
                      <div className="w-full md:w-3/5 h-72 md:h-auto relative overflow-hidden bg-zinc-950">
                        ...
                        featuredAttraction.heroMediaUrl ? (
                          ...
                          featuredAttraction.gallery?.[0]?.url ? (
                          <img
                            src={
                              featuredAttraction.heroThumbnailUrl ||
                              featuredAttraction.heroMediaUrl ||
                      ...
                      <div className="w-full md:w-2/5 p-8 md:p-12 flex flex-col justify-center relative z-20">
                        <div className="flex items-center gap-3 mb-6 flex-wrap">
                          <B2CStatusBadge
                            status={featuredAttraction.computedStatus || ""}
                          />
                        ...
                        <h3 className="text-3xl md:text-4xl font-black mb-4 font-display uppercase leading-tight">
                          {isAr
                            ? featuredAttraction.nameAr
                            : featuredAttraction.nameEn}
                        </h3>
                        <p className="text-[var(--text-secondary)] mb-8 line-clamp-3 font-medium text-sm md:text-base leading-relaxed">
                          {isAr
                            ? featuredAttraction.descriptionAr
                            : featuredAttraction.descriptionEn}
                        </p>
                        ...
                        <div className="mt-auto">
                          <B2CButton
                            variant="primary"
                            size="md"
                            className="w-full flex items-center justify-center gap-2 uppercase font-black"
                          >
                ...
                <AnimatePresence mode="popLayout">
                  {filteredAttractions.length === 0 ? (
                    <B2CEmptyState
                      ...
                      description={
                        isAr
                          ? "يرجى تغيير معايير البحث أو التصفية"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        {filteredAttractions
                          ...
                          .map((attraction, index) => {
                            ...
                            if (total === 1)
                              bentoClass +=
                                " md:col-span-4 h-[400px] md:h-[550px]";
                            else if (total === 2)
                              bentoClass +=
                                " md:col-span-2 h-[350px] md:h-[480px]";
                            else if (total === 3) {
                              if (index === 0)
                                bentoClass +=
                              ...
                              if (index === 0)
                                ...
                                bentoClass += " md:col-span-2 h-[220px]";
                          ...
                          >
                            {filteredAttractions
                              ...
                              .map((attraction, index) => (
                                <div
                                  key={attraction.id}
                                  className="min-w-[290px] md:min-w-[340px] h-[380px] snap-center shrink-0"
                                >
                                  <AttractionBrick
                                    attraction={attraction}
                                    index={index + 5}
              ...
              <B2CCard
                hoverable
                glow
                className="w-full relative overflow-hidden p-8 md:p-16 flex flex-col md:flex-row items-center justify-between gap-12 border-[rgba(75,0,143,0.3)]"
              >
                ...
                <div className="relative z-10 w-full md:w-1/2">
                  <form
                    className="flex flex-col sm:flex-row gap-3 w-full max-w-md md:ms-auto"
                    onSubmit={(e) => {
                      e.preventDefault();
                    }}
                  >
              ...
              {faqs && faqs.length > 0 && (
                <section className="max-w-3xl mx-auto w-full">
                  <div className="text-center mb-12">
                    ...
                    <p className="text-[var(--text-secondary)] font-medium">
                      {isAr
                        ? "كل ما تحتاج لمعرفته حول تجاربنا"
                  ...
                  <div className="space-y-4">
                    {faqs.map((faq: any, i: number) => (
                      <div
                        key={i}
                        className="bg-[var(--surface-default)] border border-[var(--border-level-2)] rounded-2xl overflow-hidden transition-all duration-300 hover:border-[var(--e3-purple)]"
                      >
                        <button
              ...
              <section className="w-full py-12 text-center flex flex-col items-center">
                <h2 className="text-3xl font-black mb-6 font-display uppercase">
              ...
              <B2CCard
                hoverable
                className="w-full p-8 md:p-16 flex flex-col items-center justify-center text-center border-[rgba(75,0,143,0.3)]"
              >
                ...
                <p className="text-[var(--text-secondary)] text-lg mb-8 max-w-xl font-medium">
                  {isAr
                    ? careersCta.subtitleAr || ""
                    : careersCta.subtitleEn || ""}
                </p>
                <B2CButton
                ...
                >
                  {isAr
                    ? careersCta.buttonTextAr || ""
                    : careersCta.buttonTextEn || ""}
                </B2CButton>
              </B2CCard>
            </>
          )}
        ...
        <section className="relative w-full min-h-[50vh] flex flex-col items-center justify-center overflow-hidden border-t border-[var(--border-level-2)]">
          ...
          <div className="relative z-10 w-full max-w-7xl mx-auto px-4 py-24 text-center">
            ...
            <p className="text-[var(--text-secondary)] text-lg font-bold">
              {isAr
                ? "نصنع ذكريات لا تُنسى في قطر"
                : "Creating unforgettable memories in Qatar."}
            </p>
          </div>
        </section>
      </div>
  ...
  return (
    ...
    >
      <B2CCard className="flex flex-col h-full relative group overflow-hidden border-[rgba(75,0,143,0.3)]">
        <Link
          href={`/${locale}/b2c/attractions/${attraction.slug}`}
          className="flex flex-col h-full focus:outline-none focus:ring-2 focus:ring-[var(--e3-royal-blue)]/50"
        >
          ...
          <div className="relative z-10 p-6 flex flex-col h-full justify-end min-h-[220px]">
            ...
            <div className="absolute top-6 end-6 flex flex-col gap-2 items-end text-right">
              ...
              {attraction.timingStatus &&
                attraction.timingStatus.status !== "UNKNOWN" && (
                  <div
                    className={`flex items-center gap-1.5 backdrop-blur-md px-3 py-1.5 rounded-xl text-xs font-bold border ${
                      attraction.timingStatus.status === "OPEN"
                        ? "bg-emerald-500/20 text-emerald-500 border-emerald-500/30"
                        : attraction.timingStatus.status === "CLOSING_SOON"
            ...
            <div className="mt-auto text-start">
              <h3
                className={`font-black mb-2 text-white group-hover:text-[var(--e3-royal-blue)] transition-colors line-clamp-1 font-display uppercase ${isLarge ? "text-3xl" : "text-xl"}`}
              >
              ...
              <div className="flex items-center justify-between pt-4 border-t border-white/10">
                <span className="font-mono text-xs font-bold text-white bg-[rgba(26,31,214,0.3)] px-3 py-1.5 rounded-lg backdrop-blur-md border border-[var(--e3-royal-blue)]/30 group-hover:border-[var(--e3-magenta)]/50 transition-colors uppercase tracking-wide">
                  {attraction.pricing?.[0]
                    ? `From QAR ${attraction.pricing[0].price}`
                    : "Free"}
                </span>
