...
import { useCapability } from "@/components/experience/CapabilityProvider";
...
export function AtmosphereEngine({ weather, className }: AtmosphereEngineProps) {
  ...
  const { tier, prefersReducedMotion } = useCapability();
  const { registerFrame, unregisterFrame } = useExperienceRuntime();

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas || prefersReducedMotion || tier === "TIER_C") return;
    ...
    // Particle Setup based on Weather State
    ...
    const frameId = `atmosphere-frame-${Math.random()}`;
    ...
    registerFrame(frameId, () => {
      ...
      if (weather.state === "RAIN" || weather.state === "HEAVY_RAIN") {
        ...
        // Controlled Water Accumulation Bar at Bottom
      ...
      } else if (weather.state === "HEAT") {
        // Warm thermal distortion particles
  ...
  }, [weather, tier, prefersReducedMotion, registerFrame, unregisterFrame]);

  if (prefersReducedMotion || tier === "TIER_C") return null;
