    ...
    <>
      ...
      <AnimatePresence>
        <motion.div
          ...
          exit={{ y: 80, opacity: 0 }}
          className="fixed bottom-6 inset-x-4 sm:inset-x-auto sm:left-1/2 sm:-translate-x-1/2 z-40 bg-slate-900/95 backdrop-blur-xl border border-slate-700/80 rounded-2xl shadow-2xl px-5 py-3.5 flex items-center justify-between gap-4 max-w-xl w-full"
          dir={isAr ? "rtl" : "ltr"}
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              ...
              size="sm"
              onClick={onClear}
              className="text-xs text-slate-400 hover:text-rose-400 px-2 h-8"
            ...
            <Button
              ...
              className="h-8 px-4 text-xs font-bold bg-emerald-500 hover:bg-emerald-400 text-slate-950 gap-1.5 shadow"
            >
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overflow-y-auto" dir={isAr ? "rtl" : "ltr"}>
            <motion.div
              ...
              className="relative w-full max-w-5xl bg-[var(--surface-default)] rounded-3xl border border-slate-800 shadow-2xl p-6 md:p-8 text-white my-8 max-h-[90vh] overflow-y-auto"
            >
              <div className="flex items-center justify-between pb-6 border-b border-slate-800">
                ...
                <button
                  ...
                  className="p-2 rounded-full bg-slate-800/80 text-slate-400 hover:text-white transition-colors cursor-pointer"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                <table className="w-full text-start border-collapse">
                  <thead>
                    <tr className="border-b border-slate-800">
                      <th className="p-4 text-xs font-mono uppercase text-slate-400 w-1/4 text-start">
                        ...
                        <th key={pkg.id} className="p-4 text-start align-top">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              ...
                              <span className="inline-block mt-1 text-[10px] font-mono font-bold uppercase px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400">
                  ...
                  <tbody className="divide-y divide-slate-800/60 text-xs">
                    ...
                    <tr>
                      ...
                      {comparedPackages.map(pkg => (
                        <td key={pkg.id} className="p-4 font-mono font-bold text-emerald-400 text-sm">
                    ...
                    <tr>
                      ...
                      {comparedPackages.map(pkg => (
                        <td key={pkg.id} className="p-4 text-slate-300">
                      ...
                      {comparedPackages.map(pkg => (
                        <td key={pkg.id} className="p-4 space-y-2">
                          ...
                          <Link
                            ...
                            onClick={() => setIsModalOpen(false)}
                            className="block text-center text-[11px] font-semibold text-slate-400 hover:text-white"
                          >
