...
export function PartnersClient({ initialData }: { initialData: any[] }) {
  ...
  const handleAddNew = () => {
    // QF-23 Requirement 3: New Partner records must default to hidden (isVisible: false)
    setEditForm({
      name: "",
      website: "",
      category: "TECHNOLOGY",
  ...
  const editDataQuality = useMemo(() => {
    if (!isEditing) return null;
    return analyzePartnerDataQuality(editForm, partners);
  }, [editForm, isEditing, partners]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errJson.error || (isAr ? "فشل حفظ الشريك" : "Failed to save partner"));
      }

      const data = await res.json();
      const savedPartner = data.partner;

      if (isNew) {
        setPartners([...partners, savedPartner]);
      } else {
        setPartners(partners.map((p) => (p.id === isEditing ? savedPartner : p)));
      }

      setIsEditing(null);
      router.refresh();
    } catch (err: any) {
      alert(err.message || (isAr ? "فشل حفظ الشريك" : "Failed to save partner"));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div dir={dir} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPartners.map((partner) => {
            const dq = analyzePartnerDataQuality(partner, partners);

            return (
              <div
                key={partner.id}
                className="bg-surface-default border border-border-default rounded-2xl overflow-hidden hover:border-primary/30 transition-colors flex flex-col group relative"
              >
                <div className="aspect-[3/2] bg-surface-hover flex items-center justify-center p-6 relative border-b border-border-default">
                  {partner.logoUrl ? (
                    ...
                    <div className="text-text-tertiary font-bold flex flex-col items-center gap-2">
                      <div className="w-12 h-12 rounded-xl bg-zinc-800 border border-zinc-700 text-zinc-300 font-mono font-black text-base flex items-center justify-center">
                  ...
                  <div className="absolute top-3 end-3 flex items-center gap-2">
                    {partner.isVisible ? (
                      <span
                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"
                        title={isAr ? "معروض في الموقع العام" : "Visible on public showcase"}
                      >
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        <span>{isAr ? "منشور" : "VISIBLE"}</span>
                      </span>
                    ) : (
                      <span
                        ...
                        title={isAr ? "مخفي عن الواجهة العامة" : "Hidden from public showcase"}
                      >
                        <XCircle className="w-3.5 h-3.5" />
                        <span>{isAr ? "مسودة / مخفي" : "HIDDEN"}</span>
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <div className="flex items-start justify-between gap-4 mb-2">
                    <div>
                      ...
                      <div className="text-xs font-bold text-primary mt-1 tracking-wider uppercase">
                        {(partner.category || 'PARTNER').replace(/_/g, ' ')}
                      </div>
                    </div>
                  </div>

                  {/* Staff Data Quality Warnings on Card */}
                  {!dq.isClean && (
                    <div className="my-2 p-2.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs space-y-1">
                      <div className="flex items-center gap-1.5 font-bold text-amber-400">
                        <AlertTriangle className="w-3.5 h-3.5" />
                        <span>{isAr ? `${dq.issues.length} ملاحظات تدقيق` : `${dq.issues.length} Data Warnings`}</span>
                      </div>
                      <ul className="list-disc list-inside text-[11px] text-zinc-300 ps-1">
                        {dq.issues.slice(0, 2).map((issue, idx) => (
                  ...
                  <div className="mt-auto pt-4 flex items-center justify-between border-t border-border-default">
                    {partner.website ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs text-text-tertiary hover:text-primary flex items-center gap-1"
                      >
                        <ExternalLink className="w-3 h-3" /> {isAr ? "زيارة الموقع" : "Visit Site"}
                      </a>
                    ) : (
                      <span className="text-[11px] text-zinc-500 italic">{isAr ? "بدون موقع" : "No URL"}</span>
                    )}

                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => handleEdit(partner)}
                        className="p-2 text-text-secondary hover:text-primary bg-surface-subtle hover:bg-primary/10 rounded-lg transition-colors"
                      ...
                      <button
                        ...
                        className="p-2 text-text-secondary hover:text-error bg-surface-subtle hover:bg-error/10 rounded-lg transition-colors"
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div dir={dir} className="bg-surface-default border border-border-default rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200">
              ...
              <div className="p-6 space-y-6">
                {/* Data Quality Warnings in Modal */}
                {editDataQuality && !editDataQuality.isClean && (
                  <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs space-y-1.5">
                    <div className="flex items-center gap-2 font-bold text-amber-400">
                      ...
                      <span>{isAr ? "ملاحظات جودة البيانات والتدقيق للموظفين:" : "Staff Data Quality & Audit Observations:"}</span>
                    </div>
                    <ul className="list-disc list-inside space-y-1 text-zinc-300 ps-1">
                      {editDataQuality.issues.map((issue, idx) => (
                        <li key={idx}>{isAr ? issue.messageAr : issue.messageEn}</li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <option value="AGENCY">{isAr ? "وكالات وهيئات" : "AGENCY"}</option>
                      <option value="VENUE">{isAr ? "منشآت ومواقع" : "VENUE"}</option>
                      <option value="MEDIA">{isAr ? "إعلام وصحافة" : "MEDIA"}</option>
                      <option value="CORPORATE">{isAr ? "شركات ومؤسسات" : "CORPORATE"}</option>
                      <option value="CLIENT">{isAr ? "عملاء" : "CLIENT"}</option>
                      <option value="PARTNER">{isAr ? "شركاء استراتيجيون" : "PARTNER"}</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      onChange={(e) => setEditForm({ ...editForm, orderIndex: parseInt(e.target.value, 10) || 0 })}
                      className="w-full bg-surface-hover border border-border-default rounded-xl px-4 py-2.5 text-text-primary focus:outline-none focus:border-primary text-sm"
                    />
                  </div>
                </div>
                ...
                {/* Default isVisible: false for new records */}
                <div className="p-4 rounded-xl bg-surface-hover border border-border-default space-y-2">
                  <div className="flex items-center gap-3">
                    <input 
                      type="checkbox" 
                      id="isVisible" 
                      checked={Boolean(editForm.isVisible)} 
                      onChange={(e) => setEditForm({ ...editForm, isVisible: e.target.checked })}
                      className="w-4 h-4 rounded text-primary focus:ring-primary"
                    />
                    <label htmlFor="isVisible" className="text-sm font-bold text-text-primary cursor-pointer">
                      {isAr ? "نشر الشريك وعرضه على الموقع العام" : "Publish & Display on Public Website"}
                    </label>
                  </div>
                  <p className="text-[11px] text-zinc-400 ps-7">
                    {isAr
                      ? "تبقى السجلات الجديدة مخفية بشكل افتراضي لحين اعتمادها والتحقق من الشعار والروابط من قبل الموظفين المصرح لهم."
                      : "New partner records default to hidden until verified and explicitly published by authorized staff."}
                  </p>
                </div>
              </div>
