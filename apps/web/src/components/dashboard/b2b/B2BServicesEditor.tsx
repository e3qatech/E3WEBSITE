...
export function B2BServicesEditor({ initialData }: { initialData: any }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast("B2B Services page updated successfully.", "success")
    } catch (e) {
      console.error(e)
      toast("Failed to save B2B Services page.", "error")
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6 h-full p-6">
      <AdminPageHeader 
        title="B2B Services Landing Page"
        description="Manage the main services index page hero and footer CTA."
        action={
