...
export function B2CLandingCMSView({ initialData }: { initialData: any }) {
  const [data, setData] = useState({
    hero: {
      ...
      headerEn: initialData?.hero?.headerEn || "UNFORGETTABLE EXPERIENCES IN QATAR",
      headerAr: initialData?.hero?.headerAr || "تجارب لا تُنسى في قطر",
      subHeaderEn: initialData?.hero?.subHeaderEn || "Discover top attractions, entertainment, and cultural landmarks across Qatar.",
      subHeaderAr: initialData?.hero?.subHeaderAr || "اكتشف أفضل الوجهات والأنشطة الترفيهية والمعالم الثقافية في قطر.",
      showSearch: initialData?.hero?.showSearch ?? true
    },
    featuredTitleEn: initialData?.featuredTitleEn || "Featured Attractions",
    featuredTitleAr: initialData?.featuredTitleAr || "أبرز التجارب",
    gridTitleEn: initialData?.gridTitleEn || "Explore All Attractions",
    gridTitleAr: initialData?.gridTitleAr || "استكشف جميع التجارب",
    subscribe: {
