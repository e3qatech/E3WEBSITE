      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            <button 
              onClick={goToToday}
              className="flex items-center gap-2 px-4 py-2 bg-[var(--surface-hover)] border border-[var(--border-level-2)] hover:border-[var(--border-level-3)] rounded-full text-sm font-bold text-[var(--text-primary)] whitespace-nowrap transition-colors focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none cursor-pointer"
            >
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 border rounded-full text-sm font-bold whitespace-nowrap transition-colors focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none cursor-pointer ${
                isDiscountActive 
                  ? 'bg-emerald-500 border-emerald-500 text-white font-black shadow-md' 
                  : 'bg-[var(--surface-hover)] border-[var(--border-level-2)] hover:border-[var(--border-level-3)] text-[var(--text-primary)]'
              }`}
            >
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[var(--e3-royal-blue)] hover:opacity-90 text-white border border-transparent rounded-full text-sm font-bold whitespace-nowrap transition-all shadow-md focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none cursor-pointer"
            >
            ...
            {(selectedAttractions.length > 0 || selectedEventTypes.length > 0 || isDiscountActive) && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold uppercase transition-colors bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500 hover:text-white ms-auto focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:outline-none cursor-pointer"
              >
            ...
            {(['REGULAR', 'SPECIAL', 'FESTIVAL', 'PRIVATE'] as EventType[]).map(type => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors border focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none cursor-pointer ${
                  selectedEventTypes.includes(type) 
                    ? 'bg-emerald-500 text-white border-emerald-500 shadow-md font-bold' 
                    : 'bg-[var(--surface-hover)] text-[var(--text-secondary)] border-[var(--border-level-2)] hover:border-[var(--border-level-3)] hover:text-[var(--text-primary)]'
                }`}
              >
          ...
          <div className="flex items-center gap-3 shrink-0">
            <span className="text-lg font-black uppercase tracking-widest font-satoshi text-[var(--text-primary)]">
              {format(currentDate, "MMMM yyyy", { locale: dateLocale })}
            </span>
            <div className="flex gap-1">
              <button onClick={prevMonth} className="p-2 bg-[var(--surface-hover)] hover:bg-[var(--surface-active)] rounded-full transition-colors border border-[var(--border-level-2)] focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none cursor-pointer" aria-label={isAr ? "الشهر السابق" : "Previous month"}>
                <ChevronLeft className="w-4 h-4 text-[var(--text-primary)] rtl:rotate-180" />
              </button>
              <button onClick={nextMonth} className="p-2 bg-[var(--surface-hover)] hover:bg-[var(--surface-active)] rounded-full transition-colors border border-[var(--border-level-2)] focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none cursor-pointer" aria-label={isAr ? "الشهر التالي" : "Next month"}>
                <ChevronRight className="w-4 h-4 text-[var(--text-primary)] rtl:rotate-180" />
              </button>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isSelected 
                      ? 'bg-emerald-500 border-emerald-500 text-white scale-105 shadow-[0_0_20px_rgba(16,185,129,0.3)] font-bold' 
                      : isTodayDay
                ...
                >
                  ...
                  <span className={`text-xl font-black font-satoshi ${isSelected ? 'text-white' : 'text-[var(--text-primary)]'}`}>
                    {format(day, 'd')}
                  </span>
                </button>
              );
        ...
        {attractions.length > 0 ? (
          ...
          >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all border shrink-0 focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none cursor-pointer ${
