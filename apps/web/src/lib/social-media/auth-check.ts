  ...
  try {
    ...
    switch (requiredPermission) {
      ...
      case 'CONNECT_ACCOUNTS':
        return {
          isAuthed: ['SUPER_ADMIN', 'INTEGRATION_MANAGER'].includes(userRole),
          role: userRole,
          user: session.user,
        };
      ...
      case 'MANAGE_PLACEMENTS':
        return {
          isAuthed: ['SUPER_ADMIN', 'B2C_ADMIN', 'CONTENT_MANAGER'].includes(userRole),
          role: userRole,
          user: session.user,
        };
      ...
      case 'RUN_SYNC':
        return {
          isAuthed: ['SUPER_ADMIN', 'B2C_ADMIN', 'CONTENT_MANAGER', 'EDITOR'].includes(userRole),
          role: userRole,
          user: session.user,
        };
      ...
      case 'MANAGE_GLOBAL_SETTINGS':
        return {
          isAuthed: ['SUPER_ADMIN'].includes(userRole),
          role: userRole,
          user: session.user,
        };

      default:
        return { isAuthed: false, role: userRole, user: session.user };
    }
  } catch (e) {
