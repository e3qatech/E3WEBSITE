-- CreateEnum
DO $$ BEGIN
    CREATE TYPE "SocialProviderKey" AS ENUM ('META_INSTAGRAM', 'META_FACEBOOK', 'TIKTOK', 'YOUTUBE', 'LINKEDIN', 'MANUAL');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE "SocialAccountStatus" AS ENUM ('CONNECTED', 'HEALTHY', 'EXPIRING_SOON', 'ACTION_REQUIRED', 'DISCONNECTED', 'SYNCING', 'PAUSED', 'ERROR');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE "SocialPostMediaType" AS ENUM ('IMAGE', 'VIDEO', 'REEL', 'CAROUSEL', 'TEXT', 'LINK');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE "SocialPostStatus" AS ENUM ('PUBLISHED', 'DRAFT', 'HIDDEN', 'ARCHIVED', 'UNAVAILABLE');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE "SocialModerationStatus" AS ENUM ('PENDING', 'APPROVED', 'REJECTED');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE "SocialFeedMode" AS ENUM ('AUTOMATIC', 'CURATED', 'HYBRID');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE "SocialPlacementLocation" AS ENUM ('B2C_HOME_LIVE_SOCIAL', 'B2B_HOME_CORPORATE', 'ATTRACTION_PAGE', 'BRAND_PAGE', 'CASE_STUDY_PAGE', 'CUSTOM_PAGE');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE "SocialSyncStatus" AS ENUM ('SUCCESS', 'PARTIAL_SUCCESS', 'FAILED', 'IN_PROGRESS');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE "SocialAuditAction" AS ENUM ('CREDENTIAL_UPDATE', 'ACCOUNT_CONNECT', 'ACCOUNT_DISCONNECT', 'TOKEN_REFRESH', 'SYNC_EXECUTE', 'POST_MODERATE', 'POST_MUTATE', 'FEED_UPDATE', 'PLACEMENT_UPDATE', 'SETTINGS_UPDATE');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

-- CreateTable 1: SocialProviderConfig
...
-- CreateTable 6: SocialFeedSource
...
-- CreateTable 10: SocialSyncError
...
-- CreateTable 12: SocialGlobalSettings
...
-- CreateIndexes
CREATE UNIQUE INDEX IF NOT EXISTS "SocialProviderConfig_provider_key" ON "SocialProviderConfig"("provider");
...
CREATE UNIQUE INDEX IF NOT EXISTS "SocialPost_provider_providerPostId_key" ON "SocialPost"("provider", "providerPostId");
...
CREATE UNIQUE INDEX IF NOT EXISTS "SocialFeedPost_feedId_postId_key" ON "SocialFeedPost"("feedId", "postId");
...
-- AddForeignKeys
ALTER TABLE "SocialAccount" ADD CONSTRAINT "SocialAccount_providerConfigId_fkey" FOREIGN KEY ("providerConfigId") REFERENCES "SocialProviderConfig"("id") ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE "SocialFeedSource" ADD CONSTRAINT "SocialFeedSource_feedId_fkey" FOREIGN KEY ("feedId") REFERENCES "SocialFeed"("id") ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE "SocialFeedSource" ADD CONSTRAINT "SocialFeedSource_accountId_fkey" FOREIGN KEY ("accountId") REFERENCES "SocialAccount"("id") ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE "SocialFeedPost" ADD CONSTRAINT "SocialFeedPost_feedId_fkey" FOREIGN KEY ("feedId") REFERENCES "SocialFeed"("id") ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE "SocialFeedPost" ADD CONSTRAINT "SocialFeedPost_postId_fkey" FOREIGN KEY ("postId") REFERENCES "SocialPost"("id") ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE "SocialPlacement" ADD CONSTRAINT "SocialPlacement_feedId_fkey" FOREIGN KEY ("feedId") REFERENCES "SocialFeed"("id") ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE "SocialSyncJob" ADD CONSTRAINT "SocialSyncJob_accountId_fkey" FOREIGN KEY ("accountId") REFERENCES "SocialAccount"("id") ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE "SocialSyncError" ADD CONSTRAINT "SocialSyncError_accountId_fkey" FOREIGN KEY ("accountId") REFERENCES "SocialAccount"("id") ON DELETE SET NULL ON UPDATE CASCADE;
